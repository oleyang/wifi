...
defined('SYS_PATH') or define('SYS_PATH', dirname(__FILE__) . '/system/');
...
function autoload($className){
	$array = array(
					...
					SYS_PATH . 'core/' . $className . '.class.php',
					SYS_PATH . 'lib/' . $className . '.class.php',
					SYS_PATH . 'ext/' . $className . '.class.php',				
	);
...
function run(){
	include(SYS_PATH . 'core/Config.class.php');//加载默认配置    
	config( require(BASE_PATH . 'config.php') );//加载全局配置
	Config::set('APP', array_merge(Config::get('APP'), config('APP')));	
	defined('DEBUG') or define('DEBUG', Config::get('DEBUG'));
