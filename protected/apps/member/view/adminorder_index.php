...
<div class="list_head_m">
         ...
         <table width="100%" border="0" cellpadding="0" cellspacing="1"  class="all_cont" >
           <tr>
            ...
            <td colspan="2" align="right">
               ...
               <a class="lock" href="{url('adminorder/index',array('state'=>1))}">已付款</a>
               <a class="lock" href="{url('adminorder/index',array('state'=>2))}">交易成功</a>
            </td>
          </tr>
         <form action="{url('adminorder/del')}" method="post" onSubmit="return confirm('删除不可以恢复~确定要删除吗？');"> 
          <tr>
            ...
            {loop $list $val}
              <tr id="{$val['id']}">
                  <td align="center" width="10%"><input type="checkbox" name="delid[]" value="{$val['id']}" {if $val['state']==1} disabled="disabled" {/if} /></td>
                  ...
                  <td align="center"><a class="edt" href="{url('adminorder/detail',array('id'=>$val['id']))}">查看</a>{if $val['state']!=1}<div class="del">删除</div>{/if}</td>
              </tr>
            {/loop}
            <tr>
             <td align="center" width="10%"><input type="submit" class="all_del"  value="删除"></td>
