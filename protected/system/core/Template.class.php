...
class Template {
	...
	public function __construct($config = array()) {
		$this->config = array_merge(Config::get('TPL'), (array)$config);//参数配置	
		...
		if ( $is_tpl && $this->config['TPL_CACHE_ON'] ) {
			...
			if ( empty($this->config['TPL_CACHE_TYPE']) ) {
				...
				if ( (!file_exists($cacheFile)) || (filemtime($tplFile) > filemtime($cacheFile)) ) {
					file_put_contents($cacheFile, "<?php if (!defined('MYPHP')) exit;?>" . $this->compile($tpl));//写入缓存
				}
				...
				static $cache = NULL;
				$cache = is_object($cache) ? $cache : new Cache($this->config, $this->config['TPL_CACHE_TYPE']);
