...
class App {
    ...
    public function __construct( $config=array() ) {
		...
		define('VER', '2.0.2012.1203');//框架版本号,后两段表示发布日期
		...
		require(CORE_PATH . '/Error.class.php');	//加载错误处理类

		//加载常用函数库
		if ( is_file(CORE_PATH . '/../lib/common.function.php') ) {
	...
	private function _readHtmlCache($module = '', $action = '') {	
		if ( $this->appConfig['HTML_CACHE_ON'] ) {
			require_once(CORE_PATH . '/HtmlCache.class.php');
