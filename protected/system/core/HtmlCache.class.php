...
class HtmlCache {
	...
	static public function read($module='', $action=''){	
		...
		if( empty($_SERVER['QUERY_STRING']) && preg_match("#^[a-z0-9_\-\/]+\.html$#i",$url) ){
			...
			self::$cacheFile = self::$module . '/' . self::$action . '/' . md5($_SERVER['REQUEST_URI']) . Config::$config['APP']['HTML_CACHE_SUFFIX'];		
		}
		self::$cacheFile = Config::$config['APP']['HTML_CACHE_PATH'] . self::$cacheFile;
		$dir = dirname( self::$cacheFile );
		if ( !is_dir($dir) ) {
			@mkdir($dir,0777,true);
		}
		
		if( isset(Config::$config['APP']['HTML_CACHE_RULE'][self::$module][self::$action]) ) {
			$expires = Config::$config['APP']['HTML_CACHE_RULE'][self::$module][self::$action];
		} else {
			$expires = Config::$config['APP']['HTML_CACHE_RULE'][self::$module]['*'];
		}
	...
	static private function _checkRule() {
		...
		if ( (Config::$config['APP']['HTML_CACHE_ON'] == false) || empty(Config::$config['APP']['HTML_CACHE_RULE']) ) {
			return false;
		}
		return isset(Config::$config['APP']['HTML_CACHE_RULE'][self::$module][self::$action]) || isset(Config::$config['APP']['HTML_CACHE_RULE'][self::$module]['*']);
	}
}